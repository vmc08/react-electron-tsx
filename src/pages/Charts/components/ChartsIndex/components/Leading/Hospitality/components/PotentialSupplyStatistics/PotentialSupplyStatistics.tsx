...
import { LEADING_HOSPITALITY_POTENTIAL_SUPPLY } from 'apollo/queries/chart';
...
const PotentialSupplyStatistics = () => {
  ...
  return (
    <Query<any>
      query={LEADING_HOSPITALITY_POTENTIAL_SUPPLY}
      variables={{
        token,
        exchange: marketCode,
      }}
    >
      {({ data: { leadingCharts }, loading, error }) => {
        if (!loading) {
          [
            { tooltipLabel: 'Hotel Rooms', key: 'potentialSupply' },
        ...
        return (
          <Card className="p-3" style={{ height: '100%' }} bodyStyle={{ padding: 0 }}>
            <Title level={4}>Potential Supply Statistics - Hotel Rooms</Title>
              ...
              <MiniSpinner isLoading={loading}>
                <AppDynamicChart
                  ...
                  chartLines={filteredKeys.map((key, idx) => {
                    ...
                    return {
                      key,
                      color: CHART_COLORS[chartColors[5 - idx]], // 5 is CYAN index
                    };
                  })}
